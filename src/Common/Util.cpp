...
std::string get_nix_version_display_string()
{
  ...
  // Migrate users from the old data directory "Xcoin" to the new one "Xcoin" by creating a symlink
  // Necessary (and possible) on UNIX only
  void createDataDirSymlinkIfNecessary() {
#if not defined(WIN32)
    std::string newDataDir = getDefaultDataDirectory();
    std::string oldDataDir = boost::replace_all_copy<std::string>(newDataDir, CryptoNote::CRYPTONOTE_NAME, "Xcoin");
